...
module "gateway" {
  ...
  env                = var.env
  stage              = "prod"
  callback_urls      = ["https://${var.custom_domain_name}/callback"]
  logout_urls        = ["https://${var.custom_domain_name}/logout"]
...
module "route53" {
  ...
  api_gw_domain_target = module.gateway.custom_domain_name
  api_gw_zone_id       = module.gateway.custom_domain_domain_name_configuration.hosted_zone_id
...
module "opensearch" {
  ...
  master_user_password = var.env == "prod" ? var.prod_opensearch_master_password : "testpassword"
